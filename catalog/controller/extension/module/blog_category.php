...
class ControllerExtensionModuleBlogCategory extends Controller {
	public function index() {
		...
		$categories_1 = $this->model_extension_megnor_blog_blog_category->getBlogCategories(0);
		...
		foreach ($categories_1 as $category_1) {
			...
			$categories_2 = $this->model_extension_megnor_blog_blog_category->getBlogCategories($category_1['blog_category_id']);
			
			foreach ($categories_2 as $category_2) {
				$level_3_data = array();

		// Second level
		$categories_2 = $this->model_extension_megnor_blog_blog_category->getBlogCategories(0);
						
		$level_2_data[] = array(
			'category_2_id' => $category_2['blog_category_id'],
			'name'    	=> $category_2['name'],
			'children'	=> $level_3_data,
			'href'    	=> $this->url->link('extension/megnor_blog/category', 'blogpath=' . $category_1['blog_category_id'] . '_' . $category_2['blog_category_id'])
			);					
		}
			...
			$data['categories'][] = array(
				...
				'href'     => $this->url->link('extension/megnor_blog/category', 'blogpath=' . $category_1['blog_category_id'])
			);
		}
