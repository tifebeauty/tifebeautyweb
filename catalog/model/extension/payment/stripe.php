...
class ModelExtensionPaymentStripe extends Model {		
	...
	public function getMethod($address, $total = 0) {
		...
		if (!$settings['status'] ||
			...
			return array(
				...
				'terms'			=> '',
				'title'			=> $title = html_entity_decode($settings['title_' . $language], ENT_QUOTES, 'UTF-8'),
			);
		}
	}
	...
	private function curlRequest($request, $api, $data = array()) {
		...
		curl_setopt($curl, CURLOPT_HTTPHEADER, array('Stripe-Version: 2019-03-14'));
